...
def avaliar_solucao(parametros):
    ...
    # agora cada hora do dia tem seu próprio peso
    ...
    for t in range(T):
        peso = w[t % dim]     # usa peso para hora t

        # leve não-linearidade via peso exponencial
        prod_pv = pv[t] * (1 + 0.1 * peso**2)

        m.add_constraint(grid[t] + prod_pv + des_bat[t] * eta_d ==
                         demanda[t] + carga_bat[t])

    # bateria
    m.add_constraint(SOC[0] == 5)
    for t in range(1, T):
        m.add_constraint(SOC[t] == SOC[t-1] + carga_bat[t]*eta_c - des_bat[t]/eta_d)

    # custo da rede + penalidade + pequeno ruído
    custo = m.sum(grid[t]*(0.7 + 0.05*np.random.randn()) for t in range(T))

    # penalização leve se SOC muito baixo
...
def GA(pop=50, dim=24, iter=60, mut_rate=0.15):
    ...
    for it in range(iter):
        ...
        print(f"GA iter {it}: {melhor:.2f}")

    melhor_ind = populacao[np.argmin(fitness)]
    return melhor_ind, melhor, historico
...
def EPSO(num_part=50, dim=24, max_iter=60):
    ...
    w = np.ones(dim)*0.4  # vetor mutável de pesos
    historico = []
    ...
    for it in range(max_iter):
        ...
        # adapta mutação conforme progresso
        w += np.random.normal(0, 0.05, dim)
        w = np.clip(w, 0.1, 1.0)

        historico.append(min(fitness))
        print(f"EPSO iter {it}: {min(fitness):.2f}")

    melhor = particulas[np.argmin(fitness)]
    return melhor, min(fitness), historico
