...
# Função objetivo: minimizar 2x1 + 5x2
model.obj = pyo.Objective(expr=2*model.x1 + 5*model.x2, sense=pyo.minimize)

# Restrições
model.constr1 = pyo.Constraint(expr=model.x1 + model.x2 >= 6)
model.constr2 = pyo.Constraint(expr=model.x1 + 2*model.x2 <= 18)

# Resolver usando solver GLPK
