...
if __name__ == "__main__":
    ...
    # -------------------------
    # 5️⃣ Cálculo do erro médio quadrático (MSE) entre as curvas
    # -------------------------
    n_min = min(len(hist1), len(hist2))
    h1 = hist1["melhor_valor_global"].values[:n_min]
    h2 = np.array(hist2[:n_min])

    mse = np.mean((h1 - h2)**2)

    print("\n=== COMPARAÇÃO NUMÉRICA ENTRE AS CURVAS ===")
    print(f"Erro médio quadrático (MSE) entre as curvas: {mse:.6e}")

    # Gráfico do erro quadrático ao longo das iterações
    plt.figure(figsize=(10,5))
    plt.plot(range(n_min), (h1 - h2)**2, label="Erro quadrático por iteração")
    plt.xlabel("Iterações")
    plt.ylabel("Erro quadrático (diferença²)")
    plt.title("Erro quadrático entre PSO personalizado e PySwarms")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()
    ...
    # -------------------------
    # 6️⃣ Gráfico logarítmico (para destacar convergência fina)
    # -------------------------
    plt.figure(figsize=(10,5))
    plt.semilogy(hist1["época"], hist1["melhor_valor_global"], label="PSO (NumPy + Pandas)")
    plt.semilogy(np.arange(len(hist2)), hist2, label="PSO (PySwarms)")
    plt.xlabel("Iterações")
    plt.ylabel("Melhor valor (escala log)")
    plt.title(f"Convergência (escala log) - Rastrigin ({dimensao} dimensões)")
    plt.legend()
